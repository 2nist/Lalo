...
def main():
    ...
    parser.add_argument("--max-songs", type=int, default=30, help="maximum songs to load from dataset for training")
    args = parser.parse_args()

    pairs = _load_harmonix_pairs(Path(HARMONIX_DIR), Path(AUDIO_DIR), max_songs=args.max_songs)
    ...
    cmd = (
        f'"{python_exec}" scripts/bench/section_benchmark.py --dev-only --algorithm heuristic '
