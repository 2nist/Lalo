#!/usr/bin/env python3
"""Compare NMS strategies on SALAMI: score-first vs time-first.

Writes: results/nms_strategy_compare_salami.json
"""
...
def run_all(max_songs: int = 500):
    ...
    for p in pairs:
        ...
        results.append({'id': sid, 'f1_score_nms_3.0': f_score['f1'], 'f1_time_nms_3.0': f_time['f1'], 'n_candidates': len(cands), 'kept_score': int(sum(kept_score)), 'kept_time': int(sum(kept_time))})
        print('id', sid, 'score_nms', f_score['f1'], 'time_nms', f_time['f1'])

    out = {'date': __import__('time').strftime('%Y-%m-%d %H:%M'), 'per_song': results, 'summary': {'score_nms_mean': round(mean([r['f1_score_nms_3.0'] for r in results]) if results else 0.0,4), 'time_nms_mean': round(mean([r['f1_time_nms_3.0'] for r in results]) if results else 0.0,4), 'n': len(results)}}
    (OUT / 'nms_strategy_compare_salami.json').write_text(json.dumps(out, indent=2), encoding='utf-8')
    print('Wrote results/nms_strategy_compare_salami.json')
