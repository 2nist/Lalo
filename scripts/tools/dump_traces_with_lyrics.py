#!/usr/bin/env python3
"""Dump detector traces for SALAMI ids with optional lyric JSONs applied.

Writes JSON traces to `results/traces/trace_{id}_alpha1_th0.20_lyrics.json`.
"""
...
def main():
    ...
    for p in pairs:
        ...
        lyr_path = ROOT / 'data' / 'lyrics' / f"{pid}.lyrics.json"
        lyrics = None
        if lyr_path.exists():
            try:
                lyrics = json.loads(lyr_path.read_text(encoding='utf-8'))
            except Exception as exc:
                print('failed to load lyrics for', pid, exc)

        trace_path = out_dir / f'trace_{pid}_alpha1_th{args.prob_threshold:.2f}_lyrics.json'
        try:
            res = detect_sections(audio, chords=None, weights=weights, algorithm='scored', prob_threshold=args.prob_threshold, min_section_sec=4.0, nms_gap_sec=8.0, cand_prominence=0.18, cand_sub_prominence=0.3, lyrics=lyrics, trace_path=trace_path)
            print('Wrote lyric-enabled trace for', pid, '->', trace_path)
            found += 1
        except Exception as exc:
