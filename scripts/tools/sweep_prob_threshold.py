...
Path('results/sweep_prob_threshold.json').write_text(json.dumps(results_out,indent=2))
print('Wrote results/sweep_prob_threshold.json')
#!/usr/bin/env python3
import json
from pathlib import Path
import sys
...
prob_values = [0.4, 0.5, 0.6, 0.7, 0.8]
nms = 8.0
min_sec = 4.0
beat = 0.0
seed = 42
...
for prob in prob_values:
    ...
    for p in with_audio:
        ...
        try:
            ...
            continue
        try:
            res = detect_sections(Path(audio_path), chords=None, weights=weights, min_section_sec=min_sec, nms_gap_sec=nms, beat_snap_sec=beat, algorithm='heuristic', prob_threshold=prob, random_seed=seed, downbeat_confidence_thresh=0.4)
        except Exception as e:
            print('detect error', p.get('id'), e)
            continue
        det_raw = res.get('sections', [])
        det = []
        for ss in det_raw:
    ...
    f1 = round((2 * precision * recall) / (precision + recall), 4) if (precision + recall) > 0 else 0.0
    avg_pred = round(tot_preds / count, 3) if count > 0 else 0.0
    out.append({'prob_threshold': prob, 'tp': tot_tp, 'fp': tot_fp, 'fn': tot_fn, 'precision': precision, 'recall': recall, 'f1': f1, 'avg_pred_per_song': avg_pred})
    print(f'prob={prob} tp={tot_tp} fp={tot_fp} avg_pred={avg_pred} f1={f1}')

Path('results/sweep_prob_threshold.json').write_text(json.dumps(out, indent=2))
print('Wrote results/sweep_prob_threshold.json')
