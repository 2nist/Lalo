...
for p in with_audio:
    ...
    res=detect_sections(audio,chords=None,weights=weights,min_section_sec=min_sec,nms_gap_sec=nms,beat_snap_sec=beat,algorithm='heuristic',downbeat_confidence_thresh=0.0)
...
out={'nms_gap_sec':nms,'min_section_sec':min_sec,'beat_snap_sec':beat,'tp':tot_tp,'fp':tot_fp,'fn':tot_fn,'precision':precision,'recall':recall,'avg_pred_per_song':avg_pred,'per_song':per_song}
Path('results').mkdir(parents=True,exist_ok=True)
Path('results/sections-machine-b-wave10-top.json').write_text(json.dumps(out,indent=2))
print('Wrote results/sections-machine-b-wave10-top.json')
