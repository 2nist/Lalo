#!/usr/bin/env python3
"""Run lyric-gap validation: Harmonix dev sanity + 20-song SALAMI pilot using lyric JSONs.

Writes:
 - results/harmonix_lyric_gap_validation.json
 - results/salami_lyric_gap_validation.json
"""
...
def run_harmonix_dev(max_songs: int = 30):
    ...
    for p in pairs:
        ...
        results.append({'id': p['id'], 'f1_3.0': f['f1'], 'time_s': round(time.time()-t0,2)})
    ...
    (OUT / 'harmonix_lyric_gap_validation.json').write_text(json.dumps(out, indent=2), encoding='utf-8')
    print('Wrote results/harmonix_lyric_gap_validation.json')
    return out


def run_salami_pilot(max_songs: int = 20):
    ...
    for p in pairs:
        ...
        results.append({'id': p['id'], 'f1_0.5': f05['f1'], 'f1_3.0': f30['f1'], 'time_s': round(time.time()-t0,2)})
        print('salami id', p['id'], 'F1@3.0', f30['f1'])
    summary = {
        'F1@0.5': {'mean': round(mean([r['f1_0.5'] for r in results]) if results else 0.0,4), 'n': len(results)},
        'F1@3.0': {'mean': round(mean([r['f1_3.0'] for r in results]) if results else 0.0,4), 'n': len(results)},
    }
    out = {'date': time.strftime('%Y-%m-%d %H:%M'), 'summary': summary, 'per_song': results}
    (OUT / 'salami_lyric_gap_validation.json').write_text(json.dumps(out, indent=2), encoding='utf-8')
    print('Wrote results/salami_lyric_gap_validation.json')
    return out


def main():
    print('Running lyric-gap validation: Harmonix dev (sanity) + SALAMI 20-pilot (lyrics applied where available)')
    h = run_harmonix_dev(max_songs=30)
    s = run_salami_pilot(max_songs=20)
    print('Done')
