...
# Validations
vals = {}
a = all_out.get('wave14a')
b = all_out.get('wave14b')
c = all_out.get('wave14c')
# Parity target from Wave9
wave9_target = {'tp':3,'fp':29,'f1_0.5_mean':wave9.get('summary',{}).get('detector',{}).get('F1@0.5s',{}).get('mean')}

if a:
    vals['wave14a_parity_match'] = (a['tp']==wave9_target['tp'] and a['fp']==wave9_target['fp'])
if a and b and c:
...
# write combined note
note = []
note.append('# Machine B Wave 14 Note\n')
...
Path('results/machine-b-wave14-note.md').write_text('\n'.join(note))
print('Wrote results/machine-b-wave14-note.md')
print(json.dumps({'summary':{k:{'tp':v['tp'],'fp':v['fp'],'fn':v['fn'],'precision':v['precision'],'recall':v['recall'],'avg_pred_per_song':v['avg_pred_per_song']} for k,v in all_out.items()},'validations':vals},indent=2))
