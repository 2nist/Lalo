...
for off in duration_offsets:
    ...
    count=0
    for p in with_audio:
        # robustly handle missing or bad audio paths and per-song failures
...
Path('results/sweep_duration_onset_by_id.json').write_text(json.dumps(results_out,indent=2))
print('Wrote results/sweep_duration_onset_by_id.json')
